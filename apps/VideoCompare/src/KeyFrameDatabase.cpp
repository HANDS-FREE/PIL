...
void KeyFrameDatabase::add(FrameID pKF,DBoW2::BowVector& vec)
{
    pi::ScopedMutex lock(mMutex);
    for(DBoW2::BowVector::const_iterator vit= vec.begin(), vend=vec.end(); vit!=vend; vit++)
        mvInvertedFile[vit->first].push_back(pKF);
}

void KeyFrameDatabase::erase(FrameID pKF,DBoW2::BowVector& vec)
{
