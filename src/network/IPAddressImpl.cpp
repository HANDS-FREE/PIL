...
pil_socklen_t IPv4AddressImpl::length() const
{
    return sizeof(_addr);
}
...
pi::UInt32 IPv4AddressImpl::scope() const
{
    return 0;
}
...
#else
#if __GNUC__ < 3 || defined(PIL_VXWORKS)
    struct in_addr ia;
...
{
    pi_assert (pMask->af() == AF_INET && pSet->af() == AF_INET);
...
IPv6AddressImpl::IPv6AddressImpl(const void* addr, pi::UInt32 scope): _scope(scope)
{
    std::memcpy(&_addr, addr, sizeof(_addr));
}
    ...
    _scope(0)
{
    ...
    if (prefix > 0)
    {
        _addr.s6_addr16[i++] = ByteOrder::toNetwork(static_cast<pi::UInt16>(~(0xffff >> prefix)));
    }
    while (i < 8)
    {
        _addr.s6_addr16[i++] = 0;
...
std::string IPv6AddressImpl::toString() const
{
    ...
    else
    {
        ...
        while (i < 8)
        {
            ...
            if (i < 8)
            {
                result.append(pi::buf2HexStr((u_char*)&words[i++],sizeof(UInt16)));
            }
        }
        if (_scope > 0)
        {
...
pil_socklen_t IPv6AddressImpl::length() const
{
    return sizeof(_addr);
}
...
pi::UInt32 IPv6AddressImpl::scope() const
{
    return _scope;
}
...
{
    throw pi::NotImplementedException("mask() is only supported for IPv4 addresses");
}
...
IPv6AddressImpl IPv6AddressImpl::operator & (const IPv6AddressImpl& addr) const
{
    if (_scope != addr._scope)
        throw pi::InvalidArgumentException("Scope ID of passed IPv6 address does not match with the source one.");
