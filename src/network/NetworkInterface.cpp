...
std::ostream& operator<<(std::ostream& os, const pi::NetworkInterface::MACAddress& mac)
{
    std::ios state(0);
    state.copyfmt(os);
    for (unsigned i = 0; i < mac.size(); ++i)
    {
        if (i > 0) os << pi::NetworkInterface::MAC_SEPARATOR;
...
class NetworkInterfaceImpl: public pi::RefCountedObject
{
public:
...
{
#if !defined(PIL_OS_FAMILY_WINDOWS) && !defined(PIL_VXWORKS)
    struct ifreq ifr;
...
inline const IPAddress& NetworkInterfaceImpl::address(unsigned index) const
{
    ...
    {
        std::stringstream sst;
        sst<<"No address with index "<<index;
        throw NotFoundException(sst.str());
    }
}
...
const IPAddress& NetworkInterfaceImpl::subnetMask(unsigned index) const
{
    ...
    {
        std::stringstream sst;
        sst<<"No subnet mask with index "<<index;
        throw NotFoundException(sst.str());
    }
}
...
NetworkInterface NetworkInterface::forIndex(unsigned i)
{
    if (i != NetworkInterface::NO_INDEX)
    {
        ...
        if (it != map.end())
            ...
            throw InterfaceNotFoundException("#" + itos(i));
    }
    throw InterfaceNotFoundException("#" + itos(i));
}
...
IPAddress subnetMaskForInterface(const std::string& name, bool isLoopback)
{
    ...
    else
    {
        ...
        std::string netmask;
        HKEY hKey;
#if defined(PIL_WIN32_UTF8) && !defined(PIL_NO_WSTRING)
        std::wstring usubKey;
...
#ifdef PIL_WIN32_UTF8
        UnicodeConverter::toUTF8(pAddress->FriendlyName, name);
        UnicodeConverter::toUTF8(pAddress->Description, displayName);
#else
...
#elif defined(PIL_OS_FAMILY_BSD) || (PIL_OS == PIL_OS_QNX) || (PIL_OS == PIL_OS_SOLARIS)
//
...
#include <sys/types.h>
#ifndef PIL_ANDROID // Android doesn't have <ifaddrs.h>
#include <ifaddrs.h>
#endif
#include <net/if.h>
...
NetworkInterface::Map NetworkInterface::map(bool ipOnly, bool upOnly)
{
    ...
    return result;
#else
    throw NotImplementedException("Not implemented in Android");
#endif
}
