...
const DateTime::TimeDiff DateTime::MILLISECONDS = 1000;
const DateTime::TimeDiff DateTime::SECONDS      = 1000*DateTime::MILLISECONDS;
const DateTime::TimeDiff DateTime::MINUTES      =   60*DateTime::SECONDS;
    ...
    _microsecond(microsecond)
{
    ...
    _utcTime = toUtcTime(toJulianDay(year, month, day))
            + 10*(hour*HOURS + minute*MINUTES + second*SECONDS
                  + millisecond*MILLISECONDS + microsecond);
}
...
{
    pi_assert_dbg (firstDayOfWeek >= 0 && firstDayOfWeek <= 6);
...
void DateTime::computeGregorian(double julianDay)
{
    ...
    pi_assert_dbg (_month >= 1 && _month <= 12);
    pi_assert_dbg (_day >= 1 && _day <= daysOfMonth(_year, _month));
    pi_assert_dbg (_hour >= 0 && _hour <= 23);
    pi_assert_dbg (_minute >= 0 && _minute <= 59);
    pi_assert_dbg (_second >= 0 && _second <= 59);
    pi_assert_dbg (_millisecond >= 0 && _millisecond <= 999);
    pi_assert_dbg (_microsecond >= 0 && _microsecond <= 999);
}
